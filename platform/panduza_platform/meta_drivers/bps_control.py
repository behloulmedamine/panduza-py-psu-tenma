...
class MetaDriverBpsControl(PlatformDriver):
    """ Abstract Driver with helper class to manage power supply control interface
    """
    ...
    def _PZA_DRV_config(self):
        ...
        return ChainMap(base, self._PZA_DRV_BPS_CTRL_config())
   ...
   # ---
    ...
    async def __polling_task_att_enable(self):
        ...
        while self.alive:
            ...
            await self._update_attributes_from_dict({
                "enable": {
                    "value": await self._PZA_DRV_BPS_CTRL_read_enable_value()
                }
            })
    ...
    async def __polling_task_att_volts(self):
        ...
        while self.alive:
            ...
            await self._update_attributes_from_dict({
                "volts": {
                    "goal": await self._PZA_DRV_BPS_CTRL_read_volts_goal()
                }
            })
    ...
    async def __polling_task_att_amps(self):
        ...
        while self.alive:
            ...
            await self._update_attributes_from_dict({
                "amps": {
                    "goal": await self._PZA_DRV_BPS_CTRL_read_amps_goal()
                }
            })
    ...
    async def __handle_cmds_set_enable(self, cmd_att):
        ...
        await self._prepare_update(update_obj, 
                            "enable", cmd_att,
                            "value", [bool]
                            , self._PZA_DRV_BPS_CTRL_write_enable_value
                            , self._PZA_DRV_BPS_CTRL_read_enable_value)
        await self._prepare_update(update_obj, 
                            "enable", cmd_att,
    ...
    async def __handle_cmds_set_volts(self, cmd_att):
        ...
        await self._prepare_update(update_obj, 
                            "volts", cmd_att,
                            "goal", [float, int]
                            , self._PZA_DRV_BPS_CTRL_write_volts_goal
                            , self._PZA_DRV_BPS_CTRL_read_volts_goal)
    ...
    async def __handle_cmds_set_amps(self, cmd_att):
        ...
        await self._prepare_update(update_obj, 
                            "amps", cmd_att,
                            "goal", [float, int]
                            , self._PZA_DRV_BPS_CTRL_write_amps_goal
                            , self._PZA_DRV_BPS_CTRL_read_amps_goal)
    ...
    async def __att_volts_full_update(self):
        """
        """
        min_max = await self._PZA_DRV_BPS_CTRL_volts_goal_min_max()
        await self._update_attributes_from_dict({
            "volts": {
                "min": min_max.get("min", 0),
                "max": min_max.get("max", 0),
                "goal": await self._PZA_DRV_BPS_CTRL_read_volts_goal(),
                "decimals": await self._PZA_DRV_BPS_CTRL_read_volts_decimals(),
                "polling_cycle": 1
            }
        })
    ...
    async def __att_amps_full_update(self):
        """
        """
        min_max = await self._PZA_DRV_BPS_CTRL_amps_goal_min_max()
        await self._update_attributes_from_dict({
            "amps": {
                "min": min_max.get("min", 0),
                "max": min_max.get("max", 0),
                "goal": await self._PZA_DRV_BPS_CTRL_read_amps_goal(),
                "decimals": await self._PZA_DRV_BPS_CTRL_read_amps_decimals(),
                "polling_cycle": 1
            }
        })
